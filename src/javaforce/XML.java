 ...
 * data (content) (in other words
 *
 * @XmlMixed is not supported).<br> Mixed tags are read, but when written the
...
public class XML implements TreeModelListener {
  ...
  private XMLTagPart readtag(InputStream is) {
      ...
      switch (type) {
        case XML_OPEN:
          if (tagpart.content.startsWith("?xml")) {
            if (bHeader) {
              JFLog.log("Multiple XML headers found");
              return false;
        ...
        case XML_SINGLE:
          if (tag == null) {
            //root tag
            if (bRoot) {
              JFLog.log("Multiple root tags found");
              return false;
  ...
  public void writeClass(XMLTag tag, Object obj) {
    ...
    for (int a = 0; a < childCnt; a++) {
      try {
        ...
        f = c.getField(name);
        if (f == null) {
          throw new Exception("XML:field not found:" + name);
        }
        if (childChildcnt > 0) {
          fc = f.getType();
          if (fc.isArray()) {
    ...
    for (int a = 0; a < fieldcnt; a++) {
      try {
        ...
        if (isArray) {
          ...
          }//          JFLog.log("type(array)=" + typeString);
          ...
          typeString = fs[a].toGenericString();
//          JFLog.log("type(single)=" + typeString);
          if (typeString.indexOf(" static ") != -1) {
            continue;  //ignore static fields
          }
          if (typeString.indexOf(" transient ") != -1) {
